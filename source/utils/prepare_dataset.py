...
class PTBDataset:
    ...
    def seed_bootstrap_distituent(self):
        ...
        distituent_slice_two = self.data[self.data['sentence'].str.split().str.len() > 30].str.split().str[:-2].str.join(" ")
        distituent_slice_three = self.data[self.data['sentence'].str.split().str.len() > 40].str.split().str[:-3].str.join(" ")
        distituent_slice_four = self.data[self.data['sentence'].str.split().str.len() > 50].str.split().str[:-4].str.join(" ")
        distituent_slice_five = self.data[self.data['sentence'].str.split().str.len() > 60].str.split().str[:-5].str.join(" ")
        distituent_slice_six = self.data[self.data['sentence'].str.split().str.len() > 70].str.split().str[:-6].str.join(" ")
...
if __name__ == "__main__":
    ...
    print(ptb.seed_bootstrap_constituent())