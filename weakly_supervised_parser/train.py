...
def cli_main():
    ...
    parser.add_argument("--path_to_train_sentences", type=str, default="roberta-base", 
                        help="Path to pretrained model or model identifier from huggingface.co/models")

    parser.add_argument("--model_name_or_path", type=str, default="roberta-base", 
                        help="Path to pretrained model or model identifier from huggingface.co/models")

    parser.add_argument("--output_dir", type=str, default=TRAINED_MODEL_PATH,
                        help="Path to the inside/outside model")
    ...
    parser.add_argument("--gpus", type=int, default=min(1, torch.cuda.device_count()), 
                        help="Number of GPUs to train on")

    parser.add_argument("--strategy", type=str, default="ddp", 
                        help="Different training strategies on multiple GPUs")

    parser.add_argument("--devices", type=int, default=1,
                        help="Number of devices to be used by the accelerator for the training strategy")
    ...
    parser.add_argument("--eval_batch_size", type=int, default=32, 
                        help="Number of validation samples in a batch")

    parser.add_argument("--num_workers", default=16, type=int, help="Number of workers used in the data loader")

    parser.add_argument("--inside_max_seq_length", default=256, type=int, 
                        help="The maximum total input sequence length after tokenization for the inside model")

    parser.add_argument("--outside_max_seq_length", default=64, type=int, 
                        help="The maximum total input sequence length after tokenization for the outside model")

    parser.add_argument("--num_labels", default=2, type=int, 
                        help="Binary classification, hence two classes")

    parser.add_argument("--num_self_train_iterations", default=5, type=int, 
                        help="Number of self-training iterations")

    parser.add_argument("--num_co_train_iterations", default=2, type=int, 
                        help="Number of co-training iterations")

    parser.add_argument("--upper_threshold", default=0.99, type=int, 
                        help="Threshold value to choose constituents")

    parser.add_argument("--lower_threshold", default=0.01, type=int, 
                        help="Threshold value to choose distituents")

    parser.add_argument("--num_train_rows", default=-1, type=int, 
                        help="Subset of training rows for the training")

    parser.add_argument("--num_valid_examples", default=-1, type=int, 
                        help="Subset of validation samples for the evaluation")
    ...
    train_self_trained, valid_self_trained = prepare_data_for_self_training(inside_model=inside_model, train_initial=train, valid_initial=validation, 
                                                                            threshold=args.upper_threshold, num_train_rows=args.num_train_rows, 
    ...
    train_outside, valid_outside = prepare_outside_strings(inside_model=inside_model, upper_threshold=args.upper_threshold, 
                                                           lower_threshold=args.lower_threshold, num_train_samples=args.num_train_samples, 
                                                           seed=args.seed)
